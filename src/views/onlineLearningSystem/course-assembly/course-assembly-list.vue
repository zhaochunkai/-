    ...
    <el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;">
      ...
      <el-table-column prop="name" sortable label="课程名称">
        <template slot-scope="scope">
          <el-button type="text" style="color:#00f" @click="preview(scope.row)">{{scope.row.name}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="code" label="课程号" ></el-table-column>
      <el-table-column prop="teachingWay" label="授课方式"></el-table-column>
      <el-table-column prop="usageStatus" label="状态">
        <template slot-scope="scope">
...
export default {
    ...
    methods: {
      ...
      assembly(row) {
        ...
        let courseId=row.id;
        this.$router.push({path: '/courseassembly/list/bind',query: {courseId}})
      },
      preview(row) { 
