<template>
<el-dialog class="t-dialog" :title="title" v-if="visible" :visible="visible" :close-on-click-modal="false" fullscreen @close="close" style="overflow:scroll;">
<div class="prevBox">
    <!-- 课程知识点预览-全屏 -->
    ...
    <el-col :span="18" >
      <el-card :style="`height:${height-140}px;overflow:scroll;`">
        <div slot="header" class="t-box-card-header">
          ...
          <!-- <div>
            学习要求：0.5学时 
            已学习：<el-progress :width="40" :stroke-width="2" type="circle" :percentage="25"></el-progress>
          </div> -->
        </div>
        <div class="content">
          <el-row :gutter="20">
            <el-col :span="5">
              <div v-for="(item,i) of tableData" :key="i"  @click="fileLearning(item)" class="cardList">
                <el-card :style="`background-color:${colors[item.typecode]?colors[item.typecode]: '#48bdff'}`">
                  ...
                  <!-- <div class="t-standard">资源标准：{{item.standard}}</div> -->
                </el-card>
              </div>
            </el-col>
            <el-col :span="18">
              <file-preview ref="preview" :trustedFileName="trustedFileName"></file-preview>
            </el-col>
          </el-row>
        </div>
      </el-card>
...
import { getChapterSectionResources,getKnowledgePointResources } from '@/server/online-module';
import {
  getDictionaryItems,
...
export default {
  ...
  methods: {  
    ...
    fileLearning(row) {   // 获取资源路径
      this.trustedFileName=row.trustedFileName;
    },
    refresh() {
      this._getCourseTree()
    },
