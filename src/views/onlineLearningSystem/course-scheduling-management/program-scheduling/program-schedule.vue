    ...
    <el-col :span="19" >
      ...
      <el-card v-show="JSON.stringify(nodeSelect) != '{}' && nodeSelect.level=='班级'" :style="style">
        ...
        <el-form class="t-operate-filter" :inline="true" :model="form" size="small">
          ...
          <!-- <el-form-item label="周次" >
            <el-select v-model="weekOfSemester">
              <el-option v-for="index of Number(weeks)" :key="index" :label="`第 ${index} 周 ${weekStatus[index-1]?weekStatus[index-1]: ''}`" :value="index"></el-option>
            </el-select>
          </el-form-item> -->
...
<script>
import { getGradeTree, getDefaultSemester, getTeachCalendar } from '@/server/basic-module'
import { getClassInfoById } from '@/server/training-module'
import {
  getCourseScheduling, getCourseSchedulingByWeek,
  LockCourseScheduling,getTeacherCourseScheduling
...
export default {
  ...
  methods: {
    ...
    blkj (kj, num, pointInTime) { // -------------------------遍历课节数字
      for (var i = 1; i <= num; i++) {
        let ss = ''
        if (i == 1) {
          ss = '一'
    ...
    _getCourseScheduling () { // ---------------获取某学期某班级某周的排课内容----------------
      ...
      getCourseScheduling(data).then(result => {
        ...
        let teacherArr=new Array();
        if (result.length) {
          result[0].scheduling.forEach(item => {  // 将本周所有的排课老师筛选出来，放入数组中
            if (teacherArr.indexOf(item.teacherId)==-1) {
              teacherArr.push(item.teacherId)
            }
          })
        }
        this.teachSchelding=new Array();;
    ...
    _getTeacherCourseScheduling(teacherId) { // ------------------获取老师本周的排课--------------------
      let data={
        teacherId,
     ...
     *                          操作    -----
     **********************************************************/
    chooseCell (row, column, cell, event) { // -----------------点击格子事件------------------------------------------
