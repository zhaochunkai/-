      ...
      <el-card v-show="JSON.stringify(this.nodeSelect) != '{}'&& this.nodeSelect.level!='根'" :style="style">
        ...
        <el-form class="t-operate-filter" :inline="true" :model="conditionForm" size="small">
            ...
            <el-form-item label="资源标准">
              <el-select v-model="conditionForm.Standard">
        ...
        <el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="approvalStatus" label="审核状态" width="100">
              <template slot-scope="scope">
                <el-tooltip v-show="scope.row.approvalStatus=='未通过'" :content="scope.row.rejectReason" placement="top">
                  <el-button style="color:#00f;font-size:12px;" type="text">{{scope.row.approvalStatus}}</el-button>
                </el-tooltip>
            ...
            <el-table-column  fixed="right" label="操作" width="220" align="left">
              <template slot-scope="scope">
                <el-button v-show="scope.row.approvalStatus!='通过'" type="primary" size="mini" @click="approval(scope.row)">通过审核</el-button>
    ...
    methods: {
      ...
      approval(row) {       
        ...
        this.$confirm(`此操作将通过资源的审核申请，请确认？`, '提示', {
          ...
          AcceptResource(data).then(result => {
      ...
      refuseData(data) {
        this.$confirm(`此操作将 拒绝审核资源： 【${data.title?data.title.substr(data.title.indexOf('_')+1):data.title}】，请确认？`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        }).then(() => {   
          RejectResource(data).then(result => {
