<template>
  <el-dialog class="t-dialog" v-if="visible" title="拒绝审核" :visible="visible" :close-on-click-modal="false" width="30%" @close="close">
    <el-form class="t-form-block" ref="form" :model="form" :rules="rules" label-width="100px" size="small">     
      <el-form-item label="未通过原因">
        <el-input v-model="form.rejectReason" type="textarea" :rows="6"></el-input>
      </el-form-item>
    </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      fileList: [],       
      rules: {
        reason: [{ required: true, message: '请输入拒绝审核原因', trigger: 'blur' }],
      },
    }
  },
  watch: {
