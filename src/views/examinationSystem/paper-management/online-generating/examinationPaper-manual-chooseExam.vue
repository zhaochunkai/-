<template>
<el-dialog class="t-dialog" v-if="visible" :title="`【课程：${dataForm.examinationQuestionName}】 选取试题`" :visible="visible" append-to-body :close-on-click-modal="false" width="60%" @close="close" >
  <!-- 自动组卷 -->          
  <el-form class="t-form-block" ref="form" :model="conditionForm" :inline="true" size="mini" :rules="rules">   
    <el-form-item label="所属题库" prop="examinationQuestionName">
      <el-input v-model="conditionForm.examinationQuestionName" disabled></el-input>
    </el-form-item>
    <el-form-item label="试题类型" prop="questionTypeName">
      <el-input v-model="conditionForm.questionTypeName" disabled></el-input>
    </el-form-item>
    <el-form-item label="关键词" prop="KeyWord">
      <el-input v-model="conditionForm.KeyWord"></el-input>
    </el-form-item>
    <el-form-item label="难易度" prop="difficultyLevel">
      <el-select v-model="conditionForm.difficultyLevel" filterable clearable placeholder="请选择">
  ...
  <div >
    ...
    <el-button type="primary" size="small" @click="save">选 择</el-button>
  </div>
  <!-- 表格 -->
  <el-table class="t-table-list"  ref="multipleTable" border size="small" :data="tableData" @selection-change="handleSelectionChange" tooltip-effect="dark" style="width: 100%;margin-top:10px;">
    ...
    <el-table-column prop="paperTitle" sortable label="题干" show-overflow-tooltip>
      <template slot-scope="scope">
        <el-button type="text" style="color:#00f" @click="toInfomationPage(scope.row)" v-html="decodeURI(scope.row.paperTitle)"></el-button>
      </template>
    </el-table-column>
...
import { getAssembleStrategyList,getQuestionList } from '@/server/examination-module';
  ...
  methods: {
    ...
    initialData(SkipCount = 0, pageSize = this.pageSize) {
      let data={
        ...
        ExaminationQuestionId: this.conditionForm.examinationQuestionId,  // 题库目录id，即课程id
        QuestionType: this.conditionForm.questionType,
        KeyWord: this.conditionForm.KeyWord,
        DifficultyLevelEnum: this.conditionForm.difficultyLevel,
        auditStatus: 'Pass',  // 审核通过的试题
        questionQueryType: 'NoChoose', // 查询未选择的试题
    ...
    save () {
      this.$emit('save',this.multipleSelection,this.dataForm.questionType)
      this.close();
    },
  }
}
