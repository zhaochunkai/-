<template>
<el-dialog class="t-dialog" v-if="visible" title="自动组卷" :visible="visible" :close-on-click-modal="false" width="70%" @close="close" >
  <!-- 自动组卷 -->    
    ...
    <el-row :gutter="20">
      <el-col :span="12">
        ...
        <el-form-item label="试卷名称" prop="examinationPaperName">
          <el-input v-model="form.examinationPaperName"></el-input>
        </el-form-item>
        <el-form-item label="学习项目" prop="trainingProgramsId">
          <el-input v-model="form.trainingProgramsName" disabled placeholder="根据组卷策略自动生成学习项目"></el-input>
        </el-form-item> 
      ...
      <el-col :span="12"> 
        <el-form-item label="组卷策略" prop="questionAssembleStrategyId">
          <el-select v-model="questionAssembleStrategyId" filterable clearable placeholder="请选择组卷策略">
            <el-option v-for="(item,i) of strategys" :key="i" :label="item.strategyName" :value="item.id"></el-option>
          </el-select>
        </el-form-item> 
        <el-form-item label="试卷总分" prop="grossScore">
          <el-input  v-model="form.grossScore" type="number" disabled>
            <el-button slot="append">分</el-button>
          </el-input>
        </el-form-item> 
        <el-form-item label="试卷数量" prop="examinationPaperCount">
          <el-input  v-model="form.examinationPaperCount" type="number" min="1">
            <el-button slot="append">套</el-button>
          </el-input>
        </el-form-item> 
        <el-form-item label="考试时间" prop="examinationPaperTime">
          <el-input  v-model="form.examinationPaperTime" type="number" min="1">
  ...
  <!-- 试题类型 -->
  <div class="t-header-title" v-if="dataForm.id">
    <span>试题类型</span>
  </div> 
  <!-- 表格 -->
  <el-table class="t-table-list" ref="multipleTable" border size="small" :data="questionTypeList" tooltip-effect="dark" style="width: 100%;margin-top:10px;" v-if="dataForm.id">
    ...
    <el-table-column label="试题数量" width="100">
      <template slot-scope="scope">
        {{scope.row.examinationPaper.length}}
      </template>
    </el-table-column>
    <el-table-column label="分值(数量*单题分值)" width="200">
      <template slot-scope="scope">
        {{scope.row.score?Number(scope.row.score) * scope.row.examinationPaper.length:0}}
      </template>
    </el-table-column>
...
import { 
  getAssembleStrategyList,getExaminationPaperById
...
export default {
  props: ['visible','dataForm','strategys'],
  ...
  watch: {
    visible (newVal) {
      if (newVal) {
        ...
        this.questionAssembleStrategyId=null;  
        ...
        if (this.dataForm.id) {
          this.questionAssembleStrategyId=this.dataForm.questionAssembleStrategyId;
          if (this.questionAssembleStrategyId) {
            let obj=this.strategys.find(item => {
              return item.id==this.questionAssembleStrategyId
            })
            this.$set(this.form,'trainingProgramsId',obj.trainingProgramsId)
            this.$set(this.form,'trainingProgramsName',obj.trainingProgramsName)
          }
    ...
    questionAssembleStrategyId(newVal) {
      if (newVal) {
        let obj=this.strategys.find(item => {
        ...
        this.$set(this.form,'trainingProgramsId',obj.trainingProgramsId)
        this.$set(this.form,'trainingProgramsName',obj.trainingProgramsName)
        this.$set(this.form,'courseId',obj.courseId)
        this.$set(this.form,'grossScore',obj.totalPoints)
        this.$set(this.form,'questionAssembleStrategyId',this.questionAssembleStrategyId)
      }
    },
    form: {
      handler(newName, oldName) {
        if (this.form.examinationPaperCount<1) {
  ...
  activated() {
    ...
    this._getAssembleStrategyList(); // 查询组卷策略列表
  },
  methods: {
    // -----------------------------------------查询-----------------------------------    
    _getAssembleStrategyList(SkipCount = 0, pageSize = this.pageSize) {  // 查询组卷策略列表
      let data={
        SkipCount: SkipCount, 
        MaxResultCount: 200,
      }
      getAssembleStrategyList(data).then(result => {
        this.AssembleStrategyList=result.items;
      })
    },
    ...
    save () {
      ...
      this.$set(this.form,'constructionExaminationWay','Auto');
