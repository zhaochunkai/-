...
<template>
<el-dialog class="t-dialog" v-if="visible" title="好像没有用------考务信息" :visible="visible" :close-on-click-modal="false" width="80%" @close="close" >           
  <!-- 表格 -->
  <el-table class="t-table-list" ref="multipleTable" border size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;" @selection-change="handleSelectionChange">
    ...
    <el-table-column prop="studentName" sortable label="姓名" width="100" show-overflow-tooltip>
      <template slot-scope="scope">
        <el-button type="text" style="color:#00f" @click="toInfomationPage(scope.row)">{{scope.row.studentName}}</el-button>
      </template>
    </el-table-column>
