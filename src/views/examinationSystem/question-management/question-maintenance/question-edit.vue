  ...
  <el-row :gutter="20">
    <el-col :span="16">
      ...
      <el-form class="t-form-block" ref="form" :model="form" :rules="rules" label-width="80px" size="small">
        ...
        <el-form-item label="题目" prop="paperTitle">
          ...
          <div class="t-editorContent t-textarea t-bgcolor-grey" v-html="form.paperTitle"></div>
        </el-form-item>
      </el-form>
      <!-- 选项信息 -->
      <div class="t-header-title">
  ...
  data () {
    return {
      ...
      rules: {
        ...
        paperTitle: [{required: true, message: '请输入题目', trigger: 'blur'}]
      },
    ...
    editorValue (newVal) {
      if (newVal) {
        if (this.editorContentType === 'title') {
          this.$set(this.form, 'paperTitle', this.editorValue)
  ...
  methods: {
    ...
    FilterCheckedNodes (arr, fun, nodeType) {
      arr.forEach(item => {
        this.knowledgePointWithQuestion.forEach(el => {
          if (el.knowledgePointId === item.id && el.chapterSectionKnowledgePointLevel === nodeType) {
    ...
    _getQuestionById () {
      getQuestionById(this.id).then(result => {
        result.paperTitle = decodeURI(result.paperTitle)
    ...
    editPaperTitle (type, index) {
      ...
      if (type === 'title') {
        this.editorValue = this.form.paperTitle
    ...
    changeOptions (node, trueOrFalse) { // 复选框选择知识点
      let obj = {
        ...
        chapterSectionKnowledgePointLevel: this.genarateLevelType(node.level) // 类型
      }
      if (this.form.id) {
      ...
      let finds = arr.find((item, i) => {
        return item.knowledgePointId === obj.knowledgePointId && item.chapterSectionKnowledgePointLevel === obj.chapterSectionKnowledgePointLevel
      })
      arr.forEach((item, i) => {
        if (item.knowledgePointId === obj.knowledgePointId &&
            item.chapterSectionKnowledgePointLevel === obj.chapterSectionKnowledgePointLevel &&
            !obj.trueOrFalse
    ...
    saveFun (type) {
      ...
      this.$set(questionFrom, 'paperTitle', encodeURI(questionFrom.paperTitle)) // 富文本框需转码
      // return ;
      // 保存或更新数据
      if (this.form.id) {
