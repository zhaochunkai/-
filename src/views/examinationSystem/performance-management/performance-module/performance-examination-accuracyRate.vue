<template>
  <el-drawer :title="`【${dataForm.courseName}】考试${dataForm.type && dataForm.type === 'search'?'成绩':'正确率'}`" v-if="visible" :visible="visible" :modal="false" :append-to-body="true" size="85%" @close="close">
    ...
    <el-table class="t-table-list" ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="score" label="补考成绩" width="100" align="center" v-if="dataForm.type && dataForm.type === 'search'">
        <template slot-scope="scope">
      ...
      <el-table-column prop="examCorrectRate" label="正确率" width="100" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  data () {
    return {
      ...
      print: {
        id: '#printCompletion',
        popTitle: `${sessionStorage.getItem('TenantName')}-考试试卷`,
        extraCss: '',
  ...
  methods: {
    ...
    printExamPaper (row) {
      ...
      //   this.print = {
      //     id: '#printCompletion',
      //     popTitle: `${sessionStorage.getItem('TenantName')}-作息时间表`,
      //     extraCss: '',
      //     extraHead: '<meta http-equiv="Content-Language"content="zh-cn"/>'
      //   }
      // }, 500)
