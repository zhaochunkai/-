  ...
  <el-tabs v-model="activeName" @tab-click="handleClick" type="border-card">
    <el-tab-pane label="在线学习考勤" name="onlineLearning">
      ...
      <el-table class="t-table-list" ref="onlineLearningDetailsTable" stripe size="small" :data="onlineLearningDetailData" tooltip-effect="dark" style="width: 100%;">
        ...
        <el-table-column label="人脸识别考勤记录" width="140" align="center">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" @click="viewFaceRecords(scope.row)">查 看</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页组件 -->
    ...
    <el-tab-pane label="直播学习考勤" name="livingTeaching">
      ...
      <el-table class="t-table-list" ref="livingTeachingTable" stripe size="small" :data="livingTeachingData" show-summary tooltip-effect="dark" style="width: 100%;">
        <el-table-column type="index" label="序号" width="50" align="center"></el-table-column>
        <el-table-column prop="semesterName" label="学习项目" align="center"></el-table-column>
        <el-table-column prop="trainingProgramName" label="课程" align="center"></el-table-column>
        <el-table-column prop="className" label="累计学时" align="center"></el-table-column>
        <el-table-column prop="courseName" label="应参加（次）" align="center"></el-table-column>
        <el-table-column prop="should" label="实际参加（次）" align="center"></el-table-column>
      ...
      <el-table class="t-table-list" ref="livingTeachingDetailsTable" stripe size="small" :data="livingTeachingDetailData" tooltip-effect="dark" style="width: 100%;">
        ...
        <el-table-column prop="firstEnterTime" label="第一次进入时间">
          <template slot-scope="scope">
            {{scope.row.firstEnterTime?new Date(scope.row.firstEnterTime).toLocaleDateString(): ''}}
          </template>
        </el-table-column>
        <el-table-column prop="lastLeaveTime" label="最后一次退出时间">
          <template slot-scope="scope">
            {{scope.row.lastLeaveTime?new Date(scope.row.lastLeaveTime).toLocaleDateString(): ''}}
          </template>
        </el-table-column>
        <el-table-column prop="learning" label="观看时长" width="90">
          <template slot-scope="scope">
            {{scope.row.learning}}
          </template>
        </el-table-column>
...
export default {
  props: ['studentId', 'trainingProgramId', 'studentName', 'courseList'],
  ...
  methods: {
    ...
    viewFaceRecords (row) {
      ...
      this.CheckInForm = {
        id: row.id, // 考勤记录id
        name: row.name || this.studentName, // 学生姓名
