<template>
  <div>
    <div v-if="classList && classList.length==0 && courseList && courseList.length==0">
    ...
    <div v-else>
      <el-form class="t-form-block" ref="form" label-width="60px" size="small" inline>
        <el-form-item label="班级" v-if="classList">
          <el-select v-model="classId" filterable clearable>
...
export default {
  props: ["dataForm", "courseList", "classList"],
  ...
  watch: {
    ...
    classList(newVal) {
      if (newVal && newVal.length > 0) {
        this.classId = this.dataForm.classId;
  ...
  methods: {
    ...
    queryList() {
      if (!this.classId || !this.courseId) {
        this.$message.warning("学习进度查询必须选择查询的班级和课程！");
        return;
      }
