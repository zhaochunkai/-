<template>
<el-drawer title="学习项目详细信息" v-if="visible" :visible="visible" direction="rtl" :before-close="handleClose" size="80%">
	<el-tabs v-model="activeName" @tab-click="handleClick">          
				...
				<el-row :gutter="20">
					<el-col :span="12">
						...
						<el-form-item label="招生开始时间">
							<div class="t-info-div">{{new Date(infoForm.recruitStudentsStartDate).toLocaleDateString()}}</div>
						</el-form-item> 
					...
					<el-col :span="12">
						...
						<el-form-item label="招生结束时间">
							<div class="t-info-div">{{new Date(infoForm.recruitStudentsStopDate).toLocaleDateString()}}</div>
						</el-form-item> 
					</el-col>
				</el-row>
			</el-form>        
		...
		<el-tab-pane label="学员" name="students">
			<student-list :projectId="projectId" :program="infoForm" ></student-list>
		</el-tab-pane>
	</el-tabs>   
</el-drawer>
</template>
...
import { getTrainingProgramsById,getTrainingProgramsBindStudent,getTrainingProgramsBindCourse } from '@/server/training-module'
...
export default {
		...
		visible (newVal) {
			if (newVal) {	
				...
				if (this.projectId) {
					...
					this._getTrainingProgramsBindStudent();
	...
	methods: {
      ...
      this.tableData= []
			getTrainingProgramsBindCourse(this.projectId).then(result => { // 获取项目下绑定的课程
				this.tableData=result;
			})
		},
		_getTrainingProgramsById() {  // 获取项目详细信息
			getTrainingProgramsById(this.projectId).then(result => {
				this.infoForm=Object.assign({},result);
			})
		},
		_getTrainingProgramsBindStudent() {  // 获取学生列表
			let data={
				id: this.projectId,
			}
			getTrainingProgramsBindStudent(this.projectId).then(result => {
				this.studentData=Object.assign([],result);
			})
		},
		/* 接收改变后每页多少条*/
