<template>
  <el-dialog class="t-dialog" title="设置培养方案" v-if="visible" :visible="visible" :close-on-click-modal="false" width="50%" @close="close">   
    <el-steps :active="active" finish-status="success" align-center>
      <el-step title="步骤1" :status="chooseValue.length>0?'success': 'process'" description="设置项目课程设置项目需要教授的课程"></el-step>
      <el-step title="设置2" description="设置课程培养方案：课程节次，学分等信息"></el-step>
    </el-steps>
    <!-- 设置项目课程 -->
    <div v-if="active==0" class="t-margin-top">
      <transfer-div :dataList="courseLists" :alias="alias" :chooseValue="chooseValue" @saveTransfer="getTransferDate" :isMultiple="true"></transfer-div>
    </div>
    <!-- 设置课程培养方案 -->
    <div v-else class="t-margin-top">
      ...
      <el-table class="t-table-list" ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;">
        ...
        <el-table-column prop="passScore" label="及格分数比" width="130">
          <template slot-scope="scope">
            <div class="input-div">
              <el-input v-model.number="scope.row.passScore" type="number" size="mini" min=0 max=100>
...
import { setTrainingProgramsCourseScheme,getTrainingProgramsCourseSchemeInfo,getTrainingProgramsBindCourse,updateTrainingProgramsCourse } from '@/server/training-module';
  ...
  methods: {    
    ...
    _getCourseList() { // 获取所有的课程列表
      ...
      getCourseList({MaxResultCount:200}).then(result => {  
		...
		_getTrainingProgramsBindCourse(val) { // 获取项目绑定的课程列表
    ...
    next(num) {
      ...
      if (this.active==0) {
		...
		getTransferDate(data) { // 绑定课程
			updateTrainingProgramsCourse(this.TrainingProgramsId,data).then(() => {
    ...
    save () { // 保存培养方案  
      ...
      for(let i=0;i<data.length;i++) {
        ...
        if (data[i].passScore<=0) {
          this.$alert(`课程【${data[i].courseName}】的 及格学分 必须是大于0的数字`)
          return;
        }
        if (data[i].creditHour<0) {
          this.$alert(`课程【${data[i].courseName}】的 学分 必须是不得小于0的数字`)
          return;
        }
      }
			setTrainingProgramsCourseScheme(this.TrainingProgramsId, this.tableData).then(() => {
