...
import { 
  getStudentInfoListByProgramId,CheckStudentInfo,CheckStudentInfoBatch
    ...
    renderContent(h, { node, data, store }) { 
      return (
        <span class="custom-tree-node">
          ...
          <span v-show={data.level!='根'}>   
            {node.label}
            <el-button type="primary" size="medium" v-show={data.level!=0} style="padding:2px 5px;color:#fff;margin-left:8px;background:#ff4426;border-color:#f5caca;border-radius:50%;font-size:13px;">{data.level}</el-button>
          </span>          
        </span>
      );
    },  
