<template>
  <el-dialog class="t-dialog" title="学生学习进度" v-if="visible" :visible="visible" :close-on-click-modal="false" width="80%" @close="close">
    ...
    <div style="display:flex;justify-content:space-between;">
      <div><el-button type="info" size="small" @click="next">上一个</el-button></div>
      <div class="studentInfo">
        <span>姓名：<strong style="font-size:1rem;">{{studentInfo.name}}</strong></span>
        ...
        <div v-if="courseId">
          ...
          <el-progress :percentage="studentInfo.progress" style="float:left;width:60%;margin-top:4px;"></el-progress>
        </div>

      </div>
      <div><el-button type="info" size="small" @click="next">下一个</el-button></div>
    </div>
    <!-- 树形表格 -->
    <el-table :data="tableData" style="width: 100%;margin-bottom: 20px;" row-key="id" border :tree-props="{children: 'children', hasChildren: 'hasChildren'}"
      ...
      <el-table-column prop="neededTime" label="要求学习时长" width="120">
        <template slot-scope="scope">
...
import { getLearnToTrackStudentInfo, getLearnToTrackDetails } from '@/server/statistical-module';
export default {
  ...
  methods: {
    ...
    _getTrainingProgramsBindCourse() { // 根据项目id查询课程
      this.courseList= []
      getTrainingProgramsBindCourse(this.dataForm.trainingProgramId).then(result => {
    ...
    _getLearnToTrackStudentInfo() {  
      ...
      getLearnToTrackStudentInfo(data).then(result => { // 获取追踪学生信息
    ...
    initialData() {  
      ...
      this.loading=true;
      getLearnToTrackDetails(data).then(result => { // 获取学习追踪进度详情
