...
<div>
	...
	<el-card :style="style">
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane label="机构详情" name="first">
				<el-form class="t-form-block" ref="form" :model="institutionData" :rules="rules" label-width="80px" size="small">
					<el-row>
						<el-col :span="12">
			...
			<el-tab-pane label="直属机构" name="second">
				...
				<el-form class="t-operate-filter" :inline="true" :model="conditionForm" size="small">
					<el-form-item label="机构名称">
						<el-input class="input-append" style="width:70%;"  v-model="conditionForm.InstitutionName" placeholder="请输入机构名称"></el-input>	
					</el-form-item>
				...
				<el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="institutionNumber" label="机构号">
					</el-table-column>
					<el-table-column prop="parentName" label="隶属机构">
					</el-table-column>
					<el-table-column prop="campusName" label="隶属校区">
					</el-table-column>
	...
	<!-- 新增/修改页面 -->
	<add-edit :dataForm="addEditForm" moduleLevel="下级机构" :visible.sync="addEditVisible" @save="addEditSave" ></add-edit>
</div>
</template>
<script>
import { getInstitutionList,createInstitution,updateInstitution,deleteInstitutionById,getInstitutionById,
    updateCampus,updateInstitutionStatus
...
export default {
	...
	methods: {
		...
		addEditSave (data) {
			if (data.id) {
				updateInstitution(data).then(result => {
				...
				createInstitution(data).then(result => {
					this.$message.success('保存成功');
					this.queryList();
					this.$emit('updateSchoolTree')
				}).catch(err => { 
		...
		save () {
			this.$refs.form.validate((valid) => {
				if (!valid) return;
				updateInstitution(this.institutionData).then(result => {
					this.$message.success('保存成功')
					this.queryList();
					this.$emit('updateSchoolTree')
				}).catch(err => { 
		...
		del(row) {
			this.$confirm(`此操作将 删除机构 【${row.institutionName}】， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				row.status=row.status=='Enable'?false:true
				deleteInstitutionById(row).then(() => {
		...
		// 启用停用
		isEneble(row) {
			this.$confirm(`此操作将 ${row.status=='Enable'?'停用': '启用'} 【${row.institutionName}】数据， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
