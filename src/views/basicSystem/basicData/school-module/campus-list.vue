	...
	<el-card :style="style">
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane label="学校详情" name="first">
				<el-form class="t-form-block" ref="form" :model="schoolData" :rules="rules" label-width="120px" size="small">
					<el-row>
						<el-col :span="12">
			...
			<el-tab-pane label="直属校区" name="second">
					...
					<div class="t-operate-buttons">
						...
						<!-- <el-button type="primary" size="mini" @click="addEdit()">删除</el-button>
					...
					<el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
						...
						<el-table-column prop="name" sortable label="校区名称">
							<!-- <template slot-scope="scope">{{ scope.row.date }}</template> -->
						</el-table-column>
						<el-table-column prop="code" label="校区代码">
						</el-table-column>
						<el-table-column prop="schoolName" label="校区所属单位">
						</el-table-column>
						...
						<el-table-column  fixed="right" label="操作" width="150" align="center">
							<template slot-scope="scope">
								...
								<el-button type="text" size="small" @click="isEnable(scope.row)">{{scope.row.usageStatus=='Enable'?'停用': '启用'}}</el-button>
							</template>
						</el-table-column>
					</el-table>
					<!-- 全选 -->
...
<script>
import { updateSchool, getCampusList,createCampus,updateCampus,updateCampusStatus,deleteCampusById
	...
	methods: {
		...
		save () {
				this.$refs.form.validate((valid) => {
						if (!valid) return;
						updateSchool(this.schoolData).then(result => {
								this.$emit('updateSchoolTree')
								this.$message.success('保存成功')
						}).catch(err => { 
								this.$message({
		...
		del(row) {          
				this.$confirm(`此操作将 删除校区 【${row.name}】数据， 是否继续？`, '提示', {
		...
		isEnable(row) {
				this.$confirm(`此操作将 ${row.usageStatus=='Enable'?'停用': '启用'} 【${row.name}】数据， 是否继续？`, '提示', {
						...
						updateCampusStatus(row).then((result) => {
