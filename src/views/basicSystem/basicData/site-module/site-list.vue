...
<div>
	...
	<el-card :style="style">
		...
		<el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
			...
			<el-table-column prop="classroomName" sortable label="教室名称">
			</el-table-column>
			...
			<el-table-column prop="classroomPurpose" label="教室用途">
			</el-table-column>
			<!-- <el-table-column prop="classroomType" label="教室类型"></el-table-column> -->
			<el-table-column prop="area" sortable label="面积（平方米）">
			</el-table-column>
			<el-table-column prop="capacity" sortable label="容量（人数）">
			</el-table-column>
			...
			<el-table-column  fixed="right" label="操作" width="150" align="center">
				<template slot-scope="scope">
					...
					<el-button type="text" size="small" @click="isEneble(scope.row)">{{scope.row.status!="Enable"?'启用': '停用'}}</el-button>
				</template>
			</el-table-column>
		</el-table>
		<!-- 全选 -->
	...
	<!-- 新增/修改页面 -->
	<add-edit :dataForm="addEditForm" :visible.sync="addEditVisible" @queryList="queryList" :buildingList="buildingList"></add-edit>
</div>
</template>
<script>
import {
...
export default {
	...
	methods: {
		...
		del(data) {
			...
			this.$confirm(`此操作将删除选中的数据 【${row.classroomName}】数据， 是否继续？`, '提示', {
		...
		addEdit(row) {
			...
			// this.$router.push({name: 'classroom-list-edit',params: {row}})
		},
		// tab切换
		...
		isEneble(row) {
			console.log(row)
			this.$confirm(`此操作将 ${row.status=="Enable"?'启用': '停用'} 【${row.classroomName}】数据， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				row.status=row.status!="Enable"?true:false;
				updateSiteStatus(row).then(() => {
					this.queryList();
