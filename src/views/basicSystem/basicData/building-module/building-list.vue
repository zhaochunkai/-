...
<div>
	...
	<el-card :style="style">
		...
		<el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
			...
			<el-table-column prop="buildingName" sortable label="楼栋名称">
			</el-table-column>
	...
	<!-- 新增/修改页面 -->
	<add-edit :dataForm="addEditForm" :visible.sync="addEditVisible" @save="queryList" :campusList="campusList"></add-edit>
</div>
</template>
<script>
import {
...
export default {
	...
	methods: {
		...
		del(data) {
			...
			this.$confirm(`此操作将删除选中的数据 【${row.buildingName}】数据， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {                
				deleteBuilding(row).then(() => {
		...
		addEditSave (data) {
			if (data.id) {
				updateBuilding(data).then(result => {
				...
				createBuilding(data).then(result => {
					this.$message.success('保存成功');
					this.queryList();
				}).catch(err => { 
					this.$message({
		...
		// 启用停用
		isEneble(row) {
			this.$confirm(`此操作将 ${row.status!='Enable'?'启用': '停用'} 【${row.buildingName}】数据， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				row.status=row.status!='Enable'?true:false;
				updateBuildingStatus(row).then(() => {
