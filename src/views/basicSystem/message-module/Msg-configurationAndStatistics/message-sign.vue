...
<div>
  <el-card>
    ...
    <el-row>
      <el-col :span="14">
        <el-form class="t-form-block" ref="form" :model="form" :rules="rules" label-width="150px" size="small">
...
export default {
  data () {
    return {
      ...
      questionList: [
        {ask: '签名设置规则',answer: '自定义签名字数限制为3-8个字，必须含中文，可以包含数字、英文；<br >建议使用你的『学校名』或『品牌名』，使用其他签名可能短信审核不会通过！<br >若签名内容侵犯到第三方权益必须获得第三方真实授权；'},
        {ask: '签名会出现在哪些短信中？',answer: '设置签名后，"消息推送"中列出的短信都会带上你的自定义签名，即：<br >通知提醒：考试结果、成绩查询；'},
        {ask: '短信计价规则',answer: '单条70个字符，若超出70个字符需要分成多条时，将按照67个字符每条计算；<br >一个汉字、数字、字母、空格都算一个字符；<br >含有标签（如短信签名、商品名、链接）的短信以实际发送时字符数计算。'}
      ],
  ...
  methods: {
		_getEtsGlobalSettingAll() {
			getEtsGlobalSettingAll().then(result => {
				for(let item of result) {
					...
					}else if (item.name=='Ets.Global.AutomaticallyEnableAfterApprovalSuccess') {
    ...
    save1() {
       this.$refs.form.validate((valid) => {
				if (!valid) return;
					updateEtsGlobalSetting({name: this.form.name1,value: this.form.value1}).then(result => {
						this._getEtsGlobalSettingAll();
    ...
    save2() {
       this.$refs.form.validate((valid) => {
				if (!valid) return;
					updateEtsGlobalSetting({name: this.form.name2,value: this.form.value2}).then(result => {
						this._getEtsGlobalSettingAll();
            this.$message.success('修改成功！')
					}).catch(err => { 
            this.$message({
