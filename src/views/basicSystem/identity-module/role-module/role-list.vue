...
<div>
		...
		<el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
			...
			<el-table-column  fixed="right" label="操作" width="400" align="center">
				<template slot-scope="scope">
					...
					<el-button type="primary" size="mini" @click="addEdit(scope.row)">批量添加用户</el-button>
	...
	<!-- 新增/修改页面 -->
	<add-edit :visible="addEditVisible" :dataForm="addEditForm" @changeVisibility="Visibility" @save="addEditSave" @saveAuth="saveEditAuth"></add-edit>
</div>
</template>
<script>
import { 
	getIdentityRoles,createIdentityRoles,updateIdentityRoles,deleteIdentityRoles,updateProfilePermission,
...
export default {
	...
	methods: {
		...
		showInfo() { // 点击角色名称弹出三个tab：基本信息、角色权限、角色人员
		...
		addEditSave (data) {   // 新增/修改角色
			if (data.id) {
				updateIdentityRoles(data).then(result => {
				...
				createIdentityRoles(data).then(result => {
					this.$message.success('保存成功');
					this.queryList();
				}).catch(err => { 
					this.$message({
		...
		setPermission(data) {
			this.$router.push({path: '/role/list/permission',query: {name:data.name}})
		},
		saveEditAuth(roleId,data) {  // 为角色批量添加用户
			updateIdentityUsersToRole(roleId,data).then(result => {
				this.$message.success('角色批量设置用户成功！')
			})
		},
		// saveEditPermissions(key,data) {  // 为角色设置权限
		// 	updateProfilePermission(key,data).then(result => {
