...
<el-card :style="style">
	...
	<el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
		...
		<el-table-column prop="lockoutEnd" label="用户状态" width="90">
			<template slot-scope="scope">{{!scope.row.lockoutEnd || new Date(scope.row.lockoutEnd) < new Date() ?'未锁定': '已锁定'}}</template>
		</el-table-column>
		<el-table-column  fixed="right" label="操作" width="350" align="center">
			<template slot-scope="scope">
				...
				<el-button type="text" size="small" @click="locked(scope.row)" >{{!scope.row.lockoutEnd || (new Date(scope.row.lockoutEnd) < new Date())?'锁定账户': '解除锁定'}}</el-button>
				<el-button type="text" style="color:#dc08dc;" size="small" @click="showPwdDialog(scope.row)">重置密码</el-button>
			</template>
		</el-table-column>
	</el-table>
	<!-- 全选 -->
	...
	<el-dialog title="用户角色列表" :visible.sync="rolesVisible">
		<div class="t-header-title">
			<span>姓名：{{realName}} <strong style="margin-left:1rem;font-weight:600;">登录账号：{{UserName}}</strong></span>
		</div>
		<el-table :data="rolesData" v-loading="loading" element-loading-text="数据加载中"
	...
	<!-- 新增|编辑用户 -->
	<add-edit :userForm="addEditForm" type="reset" :visible.sync="addEditVisible" @queryList="queryList"></add-edit>
</el-card>
</template>
<script>
import {
...
export default {
	...
	methods: {
		...
		userRoles(row) {  // 查看用户角色列表
			this.UserName=row.userName;
			this.realName=row.name;
			this.rolesVisible=true;
			this.rolesData= []
			getIdentityUsersRolesList(row.id).then(result => {  // 获取角色列表
		...
		del(data) {
			...
			this.$confirm(`此操作将删除选中的 【${row.name}】数据， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {                
				deleteIdentityUser(row).then(() => {
		...
		addEdit (row) {
			...
			// this.$router.push({path: "/user/list/add", query: {id: this.addEditForm.id}})
		},
		...
		// 启用停用
		locked(row) {
			this.$confirm(`此操作将 ${!row.lockoutEnd || new Date(row.lockoutEnd) < new Date()?'锁定账户': '解除锁定'} 【${row.name}】， 是否继续？`, '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				if (!row.lockoutEnd || new Date(row.lockoutEnd) < new Date()) {
					updateIdentityUserLockout(row).then(() => {
					...
					updateIdentityUserUnLock(row).then(() => {
