<template>
<el-dialog class="t-dialog" :title="form.academicYearId?'编辑用户信息': '新增用户信息'" v-if="visible" :visible="visible" :close-on-click-modal="false" width="50%" @close="close">
  <el-form class="t-form-block" ref="form" :model="form" :rules="form.id?rules2:rules" label-width="90px" size="small">
    <el-row :gutter="20">
      <el-col :span="12">
        <el-form-item label="用户类型" prop="identityUserType">
          <el-radio-group v-model="form.identityUserType" :disabled="form.id?true:false">
            <el-radio label="Faculty">教职工</el-radio>
            <!-- <el-radio label="EnterpriseUser">企业用户</el-radio> -->
          </el-radio-group>
        </el-form-item>
        <el-form-item label="工号" prop="employeeId" v-show="form.identityUserType=='Faculty'">
      ...
      <el-col :span="12">
        <el-form-item label="所属机构" prop="attachedId" v-show="form.identityUserType">
  ...
  <el-dialog title="提示" :visible.sync="treeVisible" width="30%" append-to-body :before-close="handleClose">
    <tree-checkbox ref="CheckboxTree" :treeType="form.identityUserType" :isMultiple="false" :checkBoxLevel="checkBoxLevel" :currentNodeKeysAll="currentNodeKeysAll" @transmitCheckedList="getTransmitCheckedList"></tree-checkbox>
...
export default {
  ...
  data () {
    return {
      ...
      form: {},
      rules: {
        identityUserType: [{ required: true, message: '请选择用户类型', trigger: 'blur' }],
        sex: [{ required: true, message: '请选择性别', trigger: 'blur' }],
        attachedId: [{ required: true, message: '请选择机构', trigger: 'blur' }],
        employeeId: [{ max: 50, message: '字符长度必须小于50', trigger: 'blur' }],
