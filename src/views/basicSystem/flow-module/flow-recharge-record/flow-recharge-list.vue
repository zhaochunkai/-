...
<div>
  ...
  <el-card :style="style">
    ...
    <el-table class="t-table-list" ref="multipleTable"  stripe size="small" :data="tableData" tooltip-effect="dark" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="orderStatus" sortable label="订单状态"></el-table-column>
      <el-table-column prop="payStrategy" label="订单内容" >
        <template slot-scope="scope" show-overflow-tooltip>
          单价：{{scope.row.payStrategy.unitPrice}} 元/GB<br>
          数量：{{scope.row.payStrategy.count}} GB<br>
...
export default {
  ...
  methods: {
    ...
    initDataList(SkipCount = 0, pageSize = this.pageSize) {  // 获取流量充值订单记录
      let data={
        SkipCount: SkipCount, 
