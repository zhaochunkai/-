<template>
<el-dialog class="t-dialog" :title="title" v-if="visible" :visible="visible" :close-on-click-modal="false" width="60%" @close="close">  
      ...
      <el-row :gutter="20" style="margin-left:0;margin-right:0;">
        <el-col v-for="(item,i) of listData" :key="i" :span="8" style="margin-bottom:1rem;">
          <el-button class="card" @click="chooseCard(item)">
            <div class="title-charge">{{item.count}}GB</div>
            <div class="content-charge" v-if="StrategyType=='CDN'">{{item.totalPrice}}元（{{item.singePrice}}元/GB）</div>
            <div class="content-charge" v-else>{{item.money}}元（{{item.unitPrice}}元/GB）</div>
          </el-button>
        </el-col>
      </el-row>
    </el-form-item>
  ...
  <el-dialog width="30%" title="微信扫码支付，有效时间5分钟" :visible.sync="showCode" append-to-body>
    <div class="cutdown">倒计时： <span style="color:#f00" v-if="countDownTime">{{timeFormat(formatTime_min(countDownTime))}} 分 {{timeFormat(formatTime_second(countDownTime))}} 秒</span>
    </div>
    <!-- <img :src="src">  -->
    <div class="imgs" >
...
import {
  getFluxPayStrategy,getFluxPayStrategyCode,
  getCdnAllPayStrategy,GenerateCDNPayQrCode
} from '@/server/resource-module';
export default {
  name: 'chargeDialog',
  props: ['dataForm', 'visible','title','StrategyType'],
  data () {
    return {
      TenantName: '',
      form: {},
  ...
  methods: {  
    ...
    close() {
      ...
      this.countDownTime=null;
      this.timer=null;    
      this.$emit('save',this.dataForm.id,this.StrategyType)    
      this.$emit('updateList',this.StrategyType)  
    ...
    wechatCode() {
      ...
      if (this.StrategyType=='CDN') { // CDN策略二维码
        GenerateCDNPayQrCode(this.id).then(result => {
          this.src='data:image/png;base64,'+result;
        ...
        getFluxPayStrategyCode(this.id).then(result => {
          this.src=result;
          let endTime=new Date().getTime()+5*60*1000;  // 二维码失效截止的毫秒数
          this.showCode=true;
          if (this.countDownTime) return;
