<template>
  <div>
    ...
    <!-- 学生档案 -->
    <student-profile :dataForm="addEditForm" :visible.sync="profileVisible" :nodeId="nodeId"></student-profile>
  </div>
</template>
<script>
import { getGradeTree } from '@/server/basic-module'
import { getStudentArchives } from '@/server/training-module'
  ...
  methods: {
    toInfomationPage (row) {
      ...
      this.addEditForm = {
        studentId: row.id,
        name: row.name,
        programId: row.trainingProgramsId
      }
