    ...
    >
      ...
      <el-table-column fixed="right" label="操作" width="220" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            type="primary"
            v-if="scope.row.isLook"
            size="small"
            @click="privew(scope.row)"
          >查看</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog width="80%" append-to-body :visible.sync="ResultsViewVisible">
      ...
      <el-table :data="gridData.listData" border>
        ...
        <el-table-column property="teachingWay" label="授课方式">
          <template slot-scope="scope">
            <p
              v-for="item in teachingMethod"
              :key="item.code"
            >{{scope.row.teachingWay == item.code ? item.name : ''}}</p>
          </template>
        </el-table-column>
  ...
  data() {
    return {
      ...
      passRules: {
        ...
        pass: [
          { required: true, message: "请输入选择达标状态", trigger: "blur" }
        ]
      },
      // -----审批
      ...
      rules: {
        ...
        result: [
          { required: true, message: "请输入选择判定结果", trigger: "blur" }
        ]
      },
      gridData: []
    };
