...
<div class="content" v-loading="loading"
  ...
  <!-- 文件：压缩文件 -->  
  <div class="other" v-else-if="showOther == true && trustedFileName " >
    <a style="color:#00f;" :href="rarUrl+trustedFileName" :download="trustedFileName.substr(trustedFileName.indexOf('_')+1)">文件格式不支持预览，请点击下载</a>
  </div>  
</div>
</template>
<script>
import { baseURL,fileURl } from '@/config.js'
...
export default {
  props: ['trustedFileName'],
  data() {
    return {
      url: '',
      rarUrl:baseURL+'/Api/ExternalFiles/Download?fileName=',
      height:500,
      resourceFiles: {},
      loading:true,
      /* -------------------文件预览------------------- */
  ...
  methods: {  
    // getBuffer() {
    //   console.log(this.$refs.videoPlayer.buffered)
    // },  
    enter() {
      this.showPlayBtn=true;
    ...
    _getPreviewFile() {   // 获取文件预览地址
      if (this.trustedFileName) {
        getPreviewFile(this.trustedFileName).then(res => {  // 获取本地资源并预览
          this.url=res.domainName?res.domainName+res.filePath:baseURL+'/'+res.filePath
        }).then(() => {
          this.loading= false;
          this.previewFile();
        }).catch(err => { 
    ...
    previewFile() {     // 文件预览
      if (this.trustedFileName) {
        let type=this.iconByType(this.trustedFileName).toLowerCase();   // 根据文件后缀名判断文件类型
