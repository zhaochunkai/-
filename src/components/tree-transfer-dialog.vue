<template>
<el-dialog class="t-dialog" v-if="visible" :visible="visible" :title="title" :close-on-click-modal="false" append-to-body width="60%" @close="close" >
  ...
  <el-row :gutter="20">
    <el-col style="margin-bottom:20px;" :offset="6">
      <el-tag v-show="!isMultiple" type="danger" style="padding:0 30px;color:#fff;background:#ff3f05;font-size:14px;">注意：只能选择一条数据！</el-tag>
    </el-col>
    <el-col :span="6">
      <!-- 树 -->
      <org-tree v-if="TreeType!='user'" :dataList="TreeData" @selectNode="haddleOrganizationNode"></org-tree>
      <div v-else>
        <org-tree :dataList="TreeData[0]" @selectNode="haddleOrganizationNode"></org-tree>
        <org-tree :dataList="TreeData[1]" @selectNode="haddleOrganizationNode"></org-tree>
      </div>
    </el-col>
    <el-col :span="18">
...
<script>
import { getSchoolTree, getGradeTree, getWarehouseTree, getTrainingProgramsTree, getBuildingTree, getCourseTree, getChapterTree, getSectionTree, getKnowledgeTree, getSiteListByBuilding } from '@/server/basic-module'
import { getChapterSectionList, getKnowledgePoint } from '@/server/online-module'
import { getUserListByAttach } from '@/server/identity-module'
import orgTree from '@/components/orgTree'
import transferDiv from '@/components/transfer-div.vue'
export default {
  props: ['visible', 'chooseValue', 'title', 'alias', 'isMultiple', 'TreeType', 'courseId'],
  data () {
    return {
      nodeId: '',
      nodeName: '',
  ...
  methods: {
    ...
    placeDataList () { // ----------------获取教室列表，传递数据给穿梭框
      ...
      getSiteListByBuilding(this.nodeId).then(result => {
        ...
        this.transferData.forEach(item => {
          item.name = `${item.classroomName}（${item.storey}楼）`
        })
      })
    },
    _getChapterSectionList (SkipCount = 0, pageSize = this.pageSize) { // ---获取章/节列表列表，传递数据给穿梭框
      let data = {
        SkipCount: SkipCount,
    ...
    _getKnowledgePoint (SkipCount = 0, pageSize = this.pageSize) { // ----获取知识点表列表，传递数据给穿梭框
      let data = {
        SkipCount: SkipCount,
        MaxResultCount: pageSize,
      ...
      getKnowledgePoint(data).then(result => {
        this.transferData = result.items
        this.transferData.forEach(item => {
          item.idLevel = item.id + '|knowlodge'
        })
      })
    },
    /* -------------------------------------------------------------右侧数据列表--默认值--------------------------- */
    ...
    changeMethod (item) {
      if (!this.isMultiple) {
        if (item.length > 1) {
          this.$alert('能且只能选择一项数据！')
          this.value = item.slice(item.length - 1, item.length)
        }
      }
    },
    save () { // 跟踪穿梭框数据变化
      this.$emit('saveTransfer', this.value, this.nodeName)
      this.close()
    },
    close () {
